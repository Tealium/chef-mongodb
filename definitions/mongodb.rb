...
define :mongodb_instance, mongodb_type: 'mongod', action: %i[enable start], port: 27_017, \
  ...
  unless %w[mongod shard configserver mongos].include?(type)
  ...
  configsvr = true if type == 'configserver'

  shardsvr = true if type == 'shard'
      ...
      ruby_block 'config_sharding' do
        block do
          if type == 'mongos'
            ...
            sleep(30)
            if !node['mongodb']['sharded_collections'].nil?
