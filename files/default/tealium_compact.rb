#!/usr/bin/env ruby
#
# This is a custom version of mongo_compact.rb that ignores visitor_visits_YYYY_MM
# collections.  We do minimal deletions and updates (relatively) in those collections 
# and drop them regularly so compacting them is not that useful.
#
      ...
      def each
        while db_name = @database_names.first
          ...
          while collection_name = @collection_names.first
            yield [db, collection_name] if (!collection_name.match(/visitor_visits_\d\d\d\d_\d\d/))
